...
(function () {
  ...
  function trackSite(event, props) {
    try {
      if (window.Pluks && window.Pluks.track) window.Pluks.track(event, props || {});
    } catch (_) {}
  }

  // Cross-device clipboard fake door. This deliberately asks for one thing
  // only: a valid email. The feature does not exist yet, and the copy says so
  // plainly; the purpose of the door is to measure demand before building it.
  (function crossDeviceRequest() {
    const teaser = document.getElementById("cross-device");
    const modal = document.getElementById("cross-device-modal");
    const closeBtn = document.getElementById("cross-device-modal-close");
    const successClose = document.getElementById("cross-device-success-close");
    const requestState = document.getElementById("cross-device-request-state");
    const successState = document.getElementById("cross-device-success");
    const form = document.getElementById("cross-device-form");
    const emailInput = document.getElementById("cross-device-email");
    const errorEl = document.getElementById("cross-device-error");
    if (!modal || !form || !emailInput || !errorEl) return;

    let openedFrom = "unknown";

    function hasRequested() {
      try { return localStorage.getItem("pluks_cross_device_requested") === "1"; }
    ...
    function open(location) {
      openedFrom = location || "unknown";
      if (hasRequested()) showRequested();
      else showForm();
      modal.classList.add("show");
      modal.setAttribute("aria-hidden", "false");
      errorEl.hidden = true;
      if (!hasRequested()) setTimeout(function () { emailInput.focus(); }, 50);
      trackSite("cross_device_request_opened", { cta_location: openedFrom });
    }

    function close(via) {
      if (modal.classList.contains("show") && via) {
        trackSite("cross_device_request_closed", { cta_location: openedFrom, via: via });
      }
    ...
    document.addEventListener("click", function (e) {
      const trigger = e.target.closest && e.target.closest(".cross-device-request-open");
      if (!trigger) return;
      e.preventDefault();
      open(trigger.getAttribute("data-cta-location") || "unknown");
    });

    closeBtn && closeBtn.addEventListener("click", function () { close("close_button"); });
    successClose && successClose.addEventListener("click", function () { close("success_button"); });
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (!emailOk) {
        ...
        trackSite("cross_device_request_invalid", { reason: "invalid_email" });
        return;
      }

      // Keep only a completion flag in the browser. The submitted address is
      // sent to the private request sheet and is not persisted in localStorage.
      try { localStorage.setItem("pluks_cross_device_requested", "1"); }
      catch (_) {}

      postLeadToSheet({
        type: "cross_device_access",
        email: email,
      ...
      // The email remains in the private request sheet only; analytics receives
      // the CTA location, which is enough to calculate fake-door conversion.
      trackSite("cross_device_request_submitted", { cta_location: openedFrom });
      showRequested();
    });
    ...
    if (teaser && "IntersectionObserver" in window) {
      const observer = new IntersectionObserver(function (entries) {
        ...
        trackSite("cross_device_teaser_viewed", {});
        observer.disconnect();
      }, { threshold: 0.35 });
